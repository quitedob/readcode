...
languages = {
    'en': {
        ...
        'select_output': 'Select Output Folder Address',
        'select_file': 'Select a Python File',
        'select_folder': 'Select a Folder Containing Python Files',
    ...
    'zh': {
        ...
        'select_file': '选择一个 Python 文件',
        'select_folder': '选择包含 Python 文件的文件夹',
...
class Application(tk.Tk):
    ...
    def process_single_file(self, file_path):
        ...
        self.save_to_output('single_python_file.txt', combined_content)
        messagebox.showinfo(languages[self.language]['operation_completed'], self.output_path)

    def process_folder(self, folder_path):
        # 获取所有子文件夹及其 .py 文件
        python_files_dict = self.get_python_files(folder_path)
        ...
        # 遍历每个子文件夹，保存其 .py 文件内容到对应的 .txt 文件
        for subfolder, python_files in python_files_dict.items():
            combined_content = []
            for python_file in python_files:
                combined_content.append(f"File: {os.path.basename(python_file)}\n\n")
                file_content = self.read_file_content(python_file)
                if file_content:
